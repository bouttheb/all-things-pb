...
export async function syncPodcast(): Promise<SyncResult> {
  // Support multiple podcast feeds (comma-separated) or single feed
  const feedUrls: string[] = [];
  if (process.env.PODCAST_RSS_URLS) {
    feedUrls.push(...process.env.PODCAST_RSS_URLS.split(",").map((u) => u.trim()));
  } else if (process.env.PODCAST_RSS_URL) {
    feedUrls.push(process.env.PODCAST_RSS_URL);
  }

  if (feedUrls.length === 0) {
    return { platform: "podcast", itemsAdded: 0, error: "Missing PODCAST_RSS_URL(S)" };
  }
  ...
  for (const feedUrl of feedUrls) {
    try {
      ...
      console.error(`Podcast sync failed for ${feedUrl}: ${err}`);
    }
  }
